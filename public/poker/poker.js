...
function updateOnlineLobby() {
    ...
    $("lobbyPlayers").innerHTML = onlinePlayers.map(p =>
        `<div>${p.name} — ${p.ready ? "Ready" : "Not ready"}</div>`
    ).join("");
...
function renderOpponents(showCards = false, winnerIndex = null, dealMode = false) {
    ...
    players.forEach((p, index) => {
        ...
        if (p.out) {
            div.innerHTML = `<div class="opponent-name">${p.name}</div><div class="opponent-chips">OUT</div>`;
        ...
        } else if (dealMode && p.hand.length >= 2) {
...
function newGame() {
    players.forEach((p, index) => {
        ...
        p.result = null;
        p.hand = [];
        if (index === 0) p.name = userNickname;
    });
    dealerIndex = 0;
    newRound();
}
...
function newRound() {
    ...
    boardCards = [
        new Card(deck.deal()), new Card(deck.deal()), new Card(deck.deal()),
        new Card(deck.deal()), new Card(deck.deal())
    ];

    players.forEach(p => {
        if (!p.out) p.hand = [new Card(deck.deal()), new Card(deck.deal())];
    });
...
function botAction(index) {
    ...
    if (need <= 0) {
        if (handStrength > 0.7 && Math.random() < 0.5) { action = "raise"; amount = Math.min(BOT_RAISE_AMOUNT, p.chips); }
        else action = "check";
    } else {
        const r = Math.random();
        const callThreshold = 0.3 + handStrength * 0.5;
        if (r > callThreshold) action = "fold";
        else if (handStrength > 0.75 && Math.random() < 0.35) { action = "raise"; amount = Math.min(need + BOT_RAISE_AMOUNT, p.chips); }
        else { action = "call"; amount = need; }
    }
...
function nextPhase() {
    if (phase === "preflop") {
        boardCards[0].flip(); boardCards[1].flip(); boardCards[2].flip();
        phase = "flop"; resetStreetBets(); setStatus("Flop");
        setTimeout(processTurn, 700); return;
    }
    if (phase === "flop") {
        boardCards[3].flip();
        phase = "turn"; resetStreetBets(); setStatus("Turn");
        setTimeout(processTurn, 700); return;
    }
    if (phase === "turn") {
        boardCards[4].flip();
        phase = "river"; resetStreetBets(); setStatus("River");
        setTimeout(processTurn, 700); return;
    }
...
function showdown() {
    ...
    contenders.forEach(p => {
        ...
        p.result = result;
        if (!bestResult || compareHands(result, bestResult) > 0) bestResult = result;
    });
...
function getHandInfo() {
    ...
    return evaluateSevenCards([...players[0].hand, ...visibleBoard]).name;
}
...
function evaluateSevenCards(cards) {
    ...
    let flushSuit = null;
    for (let suit in suits) { if (suits[suit].length >= 5) flushSuit = suit; }
...
function init() {
    // берём ник из URL если передан из PlayCanvas
    const params = new URLSearchParams(window.location.search);
    const nick = params.get("nickname");

    userNickname = (nick && nick.trim().length > 0)
        ? nick.trim()
        : "Guest" + Math.floor(Math.random() * 999 + 1);
