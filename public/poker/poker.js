...
let _onlinePlayers = {};   // sessionId → { nickname, chips, bet, folded, out, acted, hand, winner, result }
let _onlineBoard = [];     // строки карт с сервера
...
function _updateLobbyUI() {
    const players = Object.values(_onlinePlayers);
    $("lobbyPlayers").innerHTML = players.map(p =>
        `<div>${p.nickname} — ${p.ready ? "✓ Ready" : "Not ready"}</div>`
    ).join("") + (players.length < 2 ? "<br><div>Waiting for other players...</div>" : "");
...
function _doConnect() {
    ...
    client.joinOrCreate("PokerRoom", { nickname: userNickname })
        .then(room => {
            ...
            console.log("[Poker] joined PokerRoom:", _mySessionId);
            ...
            // ── слушаем поля состояния комнаты ───────────────
            room.state.onChange(() => {
...
function _onRoomStateChanged() {
    ...
    // переходим в игровой экран если ещё в лобби
...
function _renderOnlineGame() {
    ...
    // кнопки — активны только если мой ход
    const myTurn = _onlineCurrentTurn === _mySessionId && _onlinePhase !== "showdown" && _onlinePhase !== "waiting";
    $("betButton").disabled  = !myTurn;
    $("checkButton") && ($("checkButton").disabled = !myTurn);
...
function _renderMyCards(me) {
    ...
    const c1 = new Card(parts[0].trim());
    const c2 = new Card(parts[1].trim());
    c1.displayCard("playerCard1", true, 0);
    c2.displayCard("playerCard2", true, 100);
}
...
function _renderOnlineBoard() {
    const ids = ["card1","card2","card3","card4","card5"];
    ids.forEach((id, i) => {
        const el = $(id);
        if (!el) return;
        if (i < _onlineBoard.length) {
            const c = new Card(_onlineBoard[i]);
            c.displayCard(id, true, 0);
        } else {
            el.className = "";
            el.style.backgroundPosition = "";
        }
...
function _renderOnlineOpponents() {
    ...
    others.forEach((p, i) => {
        ...
        // показываем результат на шоудауне
...
function _getOnlineHandInfo(handStr) {
    ...
    try {
        ...
        const board = _onlineBoard.slice(0, _onlineBoard.length).map(s => new Card(s));
        return evaluateSevenCards([...hand, ...board]).name;
    } catch(e) { return null; }
}

// ════════════════════════════════════════
//  ДЕЙСТВИЯ ИГРОКА (боты + онлайн)
// ════════════════════════════════════════

function playerAction(action) {
    if (appMode === "online") {
        _onlinePlayerAction(action);
        return;
    }
    ...
    if (action === "check") {
        if (need > 0) { setStatus(`You can't check. Need to call ${need}, bet more, or fold.`); return; }
        p.acted = true;
    }
    if (action === "bet") {
        const selected = getSelectedBetAmount();
        if (selected <= 0) {
            if (need > 0) { setStatus(`You can't check. Need to call ${need} or fold.`); return; }
            p.acted = true;
        } else {
            placeBet(0, selected);
        }
...
function _onlinePlayerAction(action) {
    ...
    if (action === "fold") {
        _room.send("action", { type: "fold" });
    }
}

// ════════════════════════════════════════
//  БОТ РЕЖИМ (без изменений)
// ════════════════════════════════════════
...
class Deck {
    ...
    reset() {
        ...
        for (const suit of suits) for (const value of values) this.deck.push(value + " of " + suit);
    }
    shuffle() {
...
class Card {
    ...
    displayCard(placeHolder, flipped = true, dealDelay = 0) {
        ...
        setTimeout(() => {
            if (this.placeHolder) { this.placeHolder.classList.remove("deal-in"); this.placeHolder.style.animationDelay = ""; }
        }, dealDelay + 500);
    }
    setFace(flipped) {
        if (!this.placeHolder) return;
    ...
    flip() { if (!this.placeHolder || this.flipped) return; this.flipped = true; this.setFace(true); }
}
...
function nextActiveIndex(fromIndex) {
    for (let i = 1; i <= players.length; i++) {
        ...
        if (!p.out && !p.folded && p.chips > 0 && !p.acted) return index;
    }
    return -1;
}
...
function placeBet(playerIndex, amount) {
    ...
    if (p.bet > currentBet) {
        currentBet = p.bet;
        players.forEach((other, i) => { if (i !== playerIndex && !other.out && !other.folded && other.chips > 0) other.acted = false; });
    }
...
function newRound() {
    ...
    if (active.length <= 1) { setStatus(`${active[0].name} wins the game!`); setTimeout(newGame, 2500); return; }
...
function isBettingRoundComplete() {
    const contenders = aliveNotFolded(); if (contenders.length <= 1) return true;
    return contenders.every(p => { if (p.chips === 0) return true; if (!p.acted) return false; return p.bet === currentBet; });
}

function estimateHandStrength(hand) {
    if (!hand || hand.length < 2) return 0.3;
    const v1 = hand[0].value, v2 = hand[1].value;
    const suited = hand[0].suit === hand[1].suit, paired = v1 === v2;
    if (paired) return 0.5 + Math.min(v1 / 14, 1) * 0.4;
    let strength = (Math.max(v1,v2) / 14) * 0.5 + (Math.min(v1,v2) / 14) * 0.25;
...
function botAction(index) {
    const p = players[index], need = currentBet - p.bet;
    const hs = estimateHandStrength(p.hand);
    let action, amount = 0;
    ...
    if (action === "fold")  { p.folded = true; p.acted = true; }
    renderOpponents(false); updatePot();
